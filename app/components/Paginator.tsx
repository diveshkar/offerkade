...
export default function Paginator({
  ...
  return (
    ...
    >
      {page > 1 ? (
        <Link href={hrefFor(page - 1)} className={`${base} ${idle} gap-1.5`} rel="prev" aria-label="Previous page">
          <ChevronLeftIcon className="h-3.5 w-3.5" /> Prev
        </Link>
      ) : (
        <span className={`${base} ${disabled} gap-1.5`} aria-hidden>
          <ChevronLeftIcon className="h-3.5 w-3.5" /> Prev
        </span>
      )}
      ...
      {page < totalPages ? (
        <Link href={hrefFor(page + 1)} className={`${base} ${idle} gap-1.5`} rel="next" aria-label="Next page">
